...
;
//(function($) {
//Drupal.behaviors.myzymphonyBehavior = {
 //  attach: function () {
 //    //code starts
 // //    $('#block-zymphony ul li').click(function(e){ 
 // //    	e.preventDefault();
	// // 		$(this).find('.menu-level--1').toggle();
	// // });
 // //    //code ends
 // //    console.log(123);
 //  } 
...
jQuery(document).ready(function($){
  $(window).scroll(function() {
    if( $(window).scrollTop() > 50 ) {
      ...
      $('#block-topheaderblock .navbar-text').addClass('scroll-window');
    } else {
      $('header').css('background-color', 'transparent');
      $('#block-topheaderblock .navbar-text').removeClass('scroll-window');
    }
  });
  
  
  //Change colors in dashboard slider block according active item
  $('#dashboard-carousel').on('slide.bs.carousel', function() {
    var colorSlide = $('#dashboard-carousel .carousel-inner .item.active .color-slide').html();
    colorSlide = '#' + $.trim(colorSlide);
    $('#block-sliderdashboardblock .block-descript').css('color', colorSlide);
    $('.dashboard-carousel ').css('background-color', colorSlide);
    
  });
  
  // Contact page - adding class to radio area
  $('#edit-reason--wrapper legend').click(function(e){
    var existClass = $('.fieldset-wrapper').hasClass('open');
    if(!existClass) {
      $('.fieldset-wrapper').addClass('open');
    } else {
      $('.fieldset-wrapper').removeClass('open');
    }
  });
});;
/*!
